...
| 2026-08-27 | Admin Features | **Admin POS Order Creator & Advanced Dashboard:** Added `/tokki-admin/orders/new` (`ADMIN_ROUTES.createOrder`) with live POS product catalog picker (`AdminProductPicker`), quick-fill walk-in customer button (`⚡ Cliente en Mostrador`), instant counter payment auto-approval (`approveOrder`), and redirection to order details. Added accessible full-card touch navigation on order list and UTF-8 BOM CSV export (`exportOrdersToCsv`). Enhanced dashboard with timeframe filters (`Días`, `Semanas`, `Meses`), metric toggles, distribution charts, top products ranking, and low stock quick-filter (`?stock=low`). |
| 2026-08-27 | UI & Validation | **POS Layout Redesign, Sparkles Assets & Input Validation:** Fully redesigned `/tokki-admin/orders/new` with spacious Tokki sticker design system tokens, responsive 2-column desktop & fluid mobile layouts, Playwright visual inspection, and replaced emojis with animated `sparkles.gif`. Added comprehensive real-time and on-submit input validation for customer name, surname, cédula format (`V-12345678`), and phone numbers with live `.field-error-msg` helper states and toast warnings. Modularized into `CustomerFormCard`, `OrderSummaryCard`, and `useAdminOrderCreator`. Full test suite: **31 files / 231 tests passing (100% green)**, React Doctor score: **100/100 Great**, build & lint clean. |
